...
local NeizHubUiLibrary = {}
NeizHubUiLibrary.__index = NeizHubUiLibrary
...
function NeizHubUiLibrary:CreateWindow(config)
	config = config or {}
	local title = config.Title or "NeizHubUiLibrary"
	...
	function Window:AddTab(tabConfig)
		...
		local function activateTab()
			if activeTab then
				...
				TweenService:Create(activeTab.IconImg, TweenInfo.new(0.2), {ImageColor3 = GRAY}):Play()
		...
		-- CHANGED: makeLabel now uses RED instead of WHITE
		...
		function Tab:AddToggle(togConfig)
			...
			local state = default
			local row = makeRow(36)
			-- CHANGED: label text is RED via makeLabel
			makeLabel(row, name)
			...
			toggleTrack.BackgroundColor3 = Color3.fromRGB(55, 20, 20)
		...
		function Tab:AddSlider(sliderConfig)
			...
			sliderBox.BackgroundColor3 = Color3.fromRGB(30, 10, 10)
			sliderBox.BackgroundTransparency = 0.2
		...
		function Tab:AddDropdown(dropConfig)
			...
			-- CHANGED: selected text in dropdown box is RED
			...
			for idx, opt in ipairs(options) do
				...
				optBtn.Text = opt
				-- CHANGED: dropdown option text starts RED
	...
	local function runNotifQueue()
		...
		while #notifQueue > 0 do
			...
			local frameH = useIcon and 92 or 82
			local nFrame = Instance.new("Frame")
			nFrame.Size = UDim2.new(0, 290, 0, frameH)
			nFrame.Position = UDim2.new(1, 10, 1, -(frameH + 14))
			nFrame.BackgroundColor3 = Color3.fromRGB(14, 10, 10)
			...
			local leftBar = Instance.new("Frame", nFrame)
			...
			if useIcon then
				...
				titleLbl.Size = UDim2.new(1, -(contentOffsetX + 26), 0, 20)
				titleLbl.Position = UDim2.new(0, contentOffsetX + 26, 0, topOffsetY)
				...
				bodyLbl.Size = UDim2.new(1, -contentOffsetX, 0, 18)
				bodyLbl.Position = UDim2.new(0, contentOffsetX, 0, topOffsetY + 28)
				bodyLbl.BackgroundTransparency = 1
				bodyLbl.Text = bodyText
				bodyLbl.TextColor3 = GRAY
				...
				local bodyLbl = Instance.new("TextLabel", nFrame)
				bodyLbl.Size = UDim2.new(1, -contentOffsetX, 0, 18)
				bodyLbl.Position = UDim2.new(0, contentOffsetX, 0, topOffsetY + 26)
				bodyLbl.BackgroundTransparency = 1
				bodyLbl.Text = bodyText
				bodyLbl.TextColor3 = GRAY
				bodyLbl.Font = Enum.Font.Gotham
			...
			barBg.BackgroundColor3 = Color3.fromRGB(50, 30, 30)
			...
			local tweenIn = TweenService:Create(nFrame, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Position = UDim2.new(1, -(290 + 10), 1, -(frameH + 14))
			})
			tweenIn:Play()
			tweenIn.Completed:Connect(function()
				...
				barTween.Completed:Connect(function()
					local tweenOut = TweenService:Create(nFrame, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
