...
local BG_IMAGE = "rbxassetid://104648542611445"
local LOGO_DECAL = "rbxassetid://97042090768897"
...
local STROKE_CS = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(0.4,  Color3.fromRGB(147, 51, 234)),
	ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(255, 255, 255)),
	ColorSequenceKeypoint.new(0.6,  Color3.fromRGB(100, 180, 255)),
...
local function trackTextGradient(element)
	table.insert(textElements, element)
end

task.spawn(function()
	while true do
		currentGradIndex = currentGradIndex + 0.5
		for i = 1, #allGrads do
		...
		local idx = (math.floor(currentGradIndex) % #colors) + 1
		local nextIdx = (idx % #colors) + 1
		local alpha = (currentGradIndex % 1)
		
		local textColor = Color3.new(
			colors[idx].R * (1 - alpha) + colors[nextIdx].R * alpha,
			colors[idx].G * (1 - alpha) + colors[nextIdx].G * alpha,
			colors[idx].B * (1 - alpha) + colors[nextIdx].B * alpha
		)
		
		for i = 1, #textElements do
			local elem = textElements[i]
			if elem and elem.Parent then
				pcall(function()
					if elem:IsA("TextLabel") or elem:IsA("TextButton") then
						elem.TextColor3 = textColor
					end
				end)
			end
		end
...
function RonemeUi:CreateWindow(config)
	config = config or {}
	local title = config.Title or "RonemeUi"
	...
	function Window:AddTab(tabConfig)
		...
		local tabStroke = animStroke(TabBtn, 1.8)
		...
		TabIconImg.ImageColor3 = Color3.fromRGB(255, 105, 180)
		...
		TabNameLabel.TextColor3 = Color3.fromRGB(255, 105, 180)
		...
		trackTextGradient(TabNameLabel)
		...
		TabActiveLine.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		...
		function Tab:AddDropdown(dropConfig)
			...
			for idx, opt in ipairs(options) do
				...
				trackTextGradient(optBtn)

				optBtn.MouseEnter:Connect(function()
					TweenService:Create(optBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0.1}):Play()
				end)
